...
export const getNestedThemeValue = (propName: string, props: {
  ...
  //@ts-ignore
  const splitProps = typeof props[propName] === 'string' ? props[propName].split('.') : [props[propName]]
  ...
  splitProps.forEach((prop: string) => propValue = propValue ? propValue[prop] : undefined )
...
const parseValue = (type = 'none', prop: string | number, theme: any) => {
  switch(type) {
    case 'space':
      return Number.isInteger(prop) && theme.space[prop] ? `${theme.space[prop]}px` : prop
 ...
 */
export const getResponsiveThemeValue = (propName: string, prop: number | string | number[] | string[], theme: any, type: 'scale' | 'none' = 'none') => {
